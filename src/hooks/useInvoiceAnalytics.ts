  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<InvoiceKpis> => {
      const { data, error } = await supabase
        .from("invoices")
        .select(
          "id, invoice_type, is_final, total_amount, payment_status, payment_date, created_at, cgst_amount, sgst_amount, igst_amount"
        );
      ...
      const finalPendingList = finals.filter((i) => !isPaid(i));
      ...
      paid.forEach((inv) => {
        ...
        if (monthBuckets[mKey]) monthBuckets[mKey].received += amt(inv);
        if (weekBuckets[wKey]) weekBuckets[wKey].received += amt(inv);
      });
      ...
      return {
        ...
        finalPending: finalPendingList.reduce((s, i) => s + amt(i), 0),
